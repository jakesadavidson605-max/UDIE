...
const initialMessages: ChatMessage[] = [
  {
    ...
    source: "orchestrator",
    content:
      "## Workspace online\n\nWALL-E is ready. I can decompose a task, inspect the memory graph, route through the local/cloud gateway, or preview a safe sandbox run.\n\n**What are we solving?**",
  },
];
  ...
  return (
    <aside className="hidden w-[236px] shrink-0 flex-col border-r border-white/[.07] bg-[#0b0e16]/90 px-3 py-4 lg:flex">
      <div className="mb-7 flex items-center gap-3 px-2">
        ...
        <div>
          <div className="font-display text-[15px] font-bold tracking-[.18em] text-white">WALL-E</div>
      ...
      <div className="space-y-1">
        {["WALL-E master build", "Research notes", "Experiments"].map((item, index) => <button key={item} className="sidebar-link text-slate-400 hover:text-slate-200"><span className={`h-1.5 w-1.5 rounded-full ${index === 0 ? "bg-cyan-300" : index === 1 ? "bg-violet-300" : "bg-amber-300"}`} />{item}</button>)}
      </div>
